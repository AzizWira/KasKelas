  ...
  <header id="portfolio">
    ...
    <?php
      if(isset($_GET['aksi']) == 'delete'){ 
        ...
        $cek = $koneksi->query("SELECT * FROM pengeluaran WHERE id_pengeluaran='$id_pengeluaran'");

        if($cek->num_rows == 0){ 
          $pesan = ''; 
        }else{
          $roww = $cek->fetch_assoc();
          $nomnal = $roww['nominal'];

                    $koneksi->query("UPDATE dana SET pengeluaran=(pengeluaran - '$nomnal') WHERE id_dana='0'"); 
          $delete = $koneksi->query("DELETE FROM pengeluaran WHERE id_pengeluaran='$id_pengeluaran'");
          if($delete){
    ...
    <?php
      if(isset($_POST['tambah'])){ 
        ...
        $nominal        = $_POST['nominal'];
                  ...
                  $koneksi->query("UPDATE dana SET pengeluaran=(pengeluaran + '$nominal') WHERE id_dana='0'");
         $insert = $koneksi->query("INSERT INTO pengeluaran VALUES('NULL','$jenis','$tanggal_keluar','$nominal')") or die('Error:'.mysql_error()); 
         if($insert){ 
            echo $pesan = '<div class="w3-margin"><div class="w3-panel w3-pale-green w3-leftbar w3-border-green"><p>Pengeluaran Dana Sukses.</p></div></div>';
         }else{ 
            echo $pesan = '<div class="w3-margin"><div class="w3-panel w3-pale-red w3-leftbar w3-border-red"><p>Ups, Gagal melakukan pengeluaran dana!</p></div></div>';
                            }
        }else{
          echo $pesan='';
        }
    ...
    ?>
        ...
        <div class="w3-margin">
          ...
          <form role="form" class="form-horizontal" method="post" action="">
            <input type="hidden" name="tanggal_keluar" value="<?php echo date("Y-m-d"); ?>">
            
              <table>
                <thead>
                  <tr>
                    <th>NIK</th>
                    <th colspan="2"><?php echo ''.$row['nik'].'';?></th>
                  ...
                  <tr>
                    ...
                    <th>
                    <?php
                      $tampil = $koneksi->query("SELECT * FROM dana WHERE id_dana='0'");
                      if($tampil->num_rows == 0){
                        echo 'Rp. 0';
                      }else{
                        $rows = $tampil->fetch_assoc();
                  ...
                  <tr>
                    ...
                    <div class="w3-col" style="width:2%">&nbsp;</div>
                      <div class="w3-col" style="width:36%">
                        <input type="text" class="w3-input w3-border" id="jml" name="jenis" placeholder="Jenis Pengeluaran">
                      </div>
                    
                    <div class="w3-col" style="width:2%">&nbsp;</div>
                      <div class="w3-col" style="width:36%">
                        <input type="text" class="w3-input w3-border" id="jml" name="nominal" placeholder="Nominal" onkeypress="return hanyaAngka(event)" required>
                      </div>
                    
                    <div class="w3-col" style="width:2%">&nbsp;</div>
                      <div class="w3-col" style="width:20%">
                        <button type="submit" class="w3-input w3-border w3-button w3-white" name="tambah"><i class="fa fa-usd fa-fw"></i>Lanjut</button>
                      </div>
                    </th>
                    <th>
                      <div class="w3-col" style="width:100%">
                        <a href="../cetak.php?ni='.$nik.'" target="_blank" class="w3-input w3-border w3-button w3-white">
  ...
  <?php
      echo '
                  ...
                  <tr>
                    ...
                    <td>
                       <div class="w3-col" style="width:100%">
                          <a href="pengeluaran.php?aksi=delete&id_pengeluaran='.$row['id_pengeluaran'].'" onclick="return confirm(\'Anda yakin akan menghapus data '.$row['tanggal_keluar'].'?\')" class="w3-input w3-border w3-button w3-white">Hapus</a>
                        </div>
                    </td>
                  </tr>
                </tbody>';
